...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (TetherTokenData) {
      const tetherToken = new web3.eth.Contract(
        TetherToken.abi,
        TetherTokenData.address
      );
      this.setState({ tetherToken });
      let tethertokenbalance = await tetherToken.methods
    ...
    if (DummyTokenData) {
      const dummyToken = new web3.eth.Contract(
        DummyToken.abi,
        DummyTokenData.address
      );
      this.setState({ dummyToken });
      let dummytokenbalance = await dummyToken.methods
        .balance(this.state.account)
        .call();
    ...
    if (StakingDappData) {
      const stakingdapp = new web3.eth.Contract(
        StakingDapp.abi,
        StakingDappData.address
      );
      this.setState({ stakingdapp });
      let stakingdappbalance = await stakingdapp.methods
  ...
  stakeTokens = (amount) => {
    ...
    this.state.tetherToken.methods
      .approve(this.state.stakingdapp._address, amount)
      .send({ from: this.state.account })
      .on("transactionHash", (hash) => {
        this.state.stakingdapp.methods
          .stakeTokens(amount)
          .send({ from: this.state.account })
          .on("transactionHash", (hash) => {
            this.setState({ loading: false });
